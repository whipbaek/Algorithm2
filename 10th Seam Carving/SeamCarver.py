...
class SeamCarver:
    ...
    def findVerticalSeam(self):
        ...
        distTo = [[0 for j in range(width)] for i in range(height)]
        edgeTo = [[None for j in range(width)] for i in range(height)]
        ...
        for y in range(1, height):
            for x in range(width):
                ...
                minX = 0
                if x == 0:
                    # 좌상단 점은 인덱스를 벗어나므로 검사 x
                    distList = [distTo[y-1][x], distTo[y-1][x+1]]
                    minVal = min(distList)
                    index = distList.index(minVal)
                    if index == 0: minX = x
                    else : minX = x+1
                elif x==width-1:
                    # 우상단 점은 인덱스를 벗어나므로 검사 x
                    distList = [ distTo[y-1][x], distTo[y-1][x-1] ]
                    minVal = min(distList)
                    index = distList.index(minVal)
                    if index==0 : minX = x
                    ...
                    distList = [ distTo[y-1][x], distTo[y-1][x-1], distTo[y-1][x+1] ]
                    minVal = min(distList)
                    index = distList.index(minVal)
                    if index == 0:
                        minX = x
        ...
        # distTo 마지막 행에서 최소값 찾고, 그 x좌표 기억하기.
        # EdgeTo 에서 반복하면서 값 찾기

        minVal = 99999999
        minX = 0

        for i in range(width):
            if minVal > distTo[height-1][i]:
                minVal = distTo[height-1][i]
                minX = i


        for y in range(height-1, -1, -1):
            resultList.append(minX)
            minX = edgeTo[y][minX]
