...
class BaseballElimination:
    ...
    def isEliminated(self, teamName):
        ...
        # 1번 로직 -> maxflow & mincut 사용하지 않고 teamName이 제거되는지 확인한다.

        # 1-1 teamName의 최대 승수 구하기 -> 현재 승수 + 남은경기 수
        winNow = (self.wins[(self.team2id[teamName])])
        leftGames = (self.remaining[(self.team2id[teamName])])
        maxWin = winNow + leftGames
        resultList = []

        # 1-2 teamName 제외하고 해당팀의 현재 승수가 더 크면, 리스트에 추가하고 return
        for val in self.teams:
            if val == teamName: continue
            if self.wins[self.team2id[val]] > maxWin:
                resultList.append(val)

        # 1-3 결과 리스트에 추가 되었다면 return 하자.
        if resultList:
            return True, resultList

        # 2번 로직 -> FlowNetwork 생성.

        g8m = FlowNetwork(len(teamName))

        # FlowEdge(시작점, 도착점, 가중치)
        # g8m.addEdge(FlowEdge())
