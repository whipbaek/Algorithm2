...
def solveManhattan(initialBoard):
    pq = PriorityQueue()
    pq.put([0 + initialBoard.manhattan(), initialBoard, 0, None]) # manhattan 거리, 보드, 이동횟수, 직전 상태
    answer = []
    while True :
        ...
        answer.append(minNode)
        print('<standard : ', temp[0],', ', temp[2], '>')
        print(minNode)
        if minNode.isGoal():
            return answer
        else:
            nlist = minNode.neighbors()
            for val in nlist:
                if val.__eq__(temp[3]) : continue
                else:
                    print('insertion : ', temp[2] + 1 + val.manhattan(), ', ', temp[2] + 1)
                    pq.put([temp[2] + 1 + val.manhattan(), val, temp[2] + 1, minNode])
                    # print(val)
