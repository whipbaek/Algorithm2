...
def sap(g, aList, bList):
    ...
    # initalize visited HashMap (Saved Vertex and Value)

    answer = []


    for val in q:
        if (val[0] in visitedA) and (val[0] in visitedB):
            return (val[0], 0)

    while q:
        n = q.pop(0) # n = [정점, 어디서 왔는지, 거리]
        if n[2] >= sapLength:
            break

        if n[1] == 'a':
            for i in g.adj[n[0]]:
                if i not in visitedA:
                    visitedA[i] = n[2]+1 # a로부터 방문을 했으며, 그 값은 n[2] 이다.
                    q.append((i, 'a', n[2]+1))
                if i in visitedB:
                    if sapLength > visitedA[i] + visitedB[i]:
                        sapLength = visitedA[i] + visitedB[i]
                        answer = (i, sapLength)
        elif n[1] == 'b':
            for i in g.adj[n[0]]:
                if i not in visitedB:
                    visitedB[i] = n[2]+1
                    q.append((i, 'b', n[2] + 1))
                if i in visitedA:
                    if sapLength > visitedB[i] + visitedA[i]:
                        sapLength = visitedB[i] + visitedA[i]
                        answer = (i, sapLength)
