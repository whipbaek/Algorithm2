...
def mstPrimEager(g):
    ...
    edgesInMST = [] # 정답 MST 배열
    weightSum = 0
    included = [False] * g.V # MST 포함 여부 체크 배열
    pq = IndexMinPQ(g.V) # 정점개의 인덱스로 초기화
    for val in g.adj[0]:
        pq.insert(val.w, val) # idx, key
    included[0] = True
    ...
    while not pq.isEmpty() and len(edgesInMST) < g.V-1:
        ...
        edge = e[0]
        idx = e[1]
        if included[edge.v] and included[edge.w] : continue
        edgesInMST.append(edge)
        weightSum += edge.weight;

        if not included[edge.v]: target = edge.v
        if not included[edge.w]: target = edge.w

        included[target] = True
        for val in g.adj[target]: # val -> 7의 간선 정보들
            to = val.other(target) # 7 의 간선 정보중 7 이 아닌 도착 정보 값
            if not included[to]: #cycle을 만들지 않는다면, 해당 친구가 포함되지 않는다면
                if pq.contains(to) :
                    if val.weight < pq.keyOf(to).weight:
                        pq.decreaseKey(to, val)
                else:
                    pq.insert(to, val)
    return edgesInMST, weightSum
